...
export default function Projectss({ isPending }) {
    ...
    const getClientName = (id) => {
        ...
        console.log(clients);
        return (
            clients?.find((c) => parseInt(c.id) === parseInt(id))?.name || id
        );
    };
    ...
    const handleEdit = () => {
        ...
        if (
            ...
            if (projectDetails.id) {
                ...
                editProjects(projectDetails.id, editedProjectDetails).then(
                    (res) => {
                        dispatch(
                            editProjectAction({
                                ...res.data,
    ...
    React.useEffect(() => {
        getAllClients().then((res) => {
            ...
            dispatch(
                setClients(res.data.filter((c) => parseInt(c.isPending) === 0)),
            );
        });
    }, []);
                                                                ...
                                                                {isPending && (
                                                                    <>
                                                                        ...
                                                                        <Tooltip title="Paid Project">
                                                                            <IconButton
                                                                                onClick={() => {
                                                                                    handleOpenConfirmApproveProject(
                                                                                        row,
                                                                                    );
                                                                                }}
                                                                            >
